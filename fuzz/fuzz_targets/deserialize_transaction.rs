...
fn do_test(data: &[u8]) {
    let tx_result: Result<elements::blockdata::transaction::Transaction, _> = elements::consensus::encode::deserialize(data);
    match tx_result {
        Err(_) => {},
        Ok(mut tx) => {
            let ser = elements::consensus::encode::serialize(&tx);
            assert_eq!(&ser[..], data);
            let len = ser.len();
            ...
            let no_witness_len = elements::consensus::encode::serialize(&tx).len();
